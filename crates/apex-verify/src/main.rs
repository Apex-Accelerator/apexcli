...
fn main() {
    ...
    // открываем новый терминал только если verified флаг не существует
    let verified_path = PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string()))
        .join(".apex").join("verified");
